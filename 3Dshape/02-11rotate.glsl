...
//https://juejin.cn/post/7219189767672627255#heading-14
//绕v向量方向上的旋转rad弧度
...
//     return mat3(c + (1. - c) * m.x * m.x, (1. - c) * m.x * m.y - s * m.z, (1. - c) * m.x * m.z + s * m.y, (1. - c) * m.x * m.y + s * m.z, c + (1. - c) * m.y * m.y, (1. - c) * m.y * m.z - s * m.x, (1. - c) * m.x * m.z - s * m.y, (1. - c) * m.y * m.z + s * m.x, c + (1. - c) * m.z * m.z);
// }
//场景形状组合
vec2 scene(vec3 p) {
    vec3 p1 = rotation(vec3(1., 1., 1.), iTime * PI * 0.5) * (p - vec3(0., 1., -1.));
