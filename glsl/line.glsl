...
vec3 hsv2rgb2(vec3 c, float k) {
    ...
    vec3 p = smoothstep(0. + k, 1. - k, .5 + .5 * cos((c.xxx + K.xyz) * radians(360.)));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    for(int i = 0; i < 3; ++i) {
        ...
        col += 1. / pow(max(1., d1 * scale), .8 + .1 * sin(radians(245.) * iTime + radians(360.) * float(i) / 3.)) * (vec3(1.) + hsv2rgb2(vec3(colorfreqs[i] * x, 1., 1.), .07));
    }
